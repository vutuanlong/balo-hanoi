...
function zw_breadcrumbs() {
  $delimiter = '<span class="delimiter"><i class="fa fa-chevron-right" aria-hidden="true"></i></span>';
  $before = '<span class="current">';
  $after = '</span>';

  if ( !is_home() && !is_front_page() || is_paged() ) {

    echo '<div xmlns:v="https://rdf.data-vocabulary.org/#"  id="crumbs" class="crumbs">';

    global $post;
    $homeLink = home_url();
    echo '<span typeof="v:Breadcrumb"><a rel="v:url" property="v:title" class="crumbs-home" href="' . $homeLink . '"><i class="tieicon-home"></i>' . __( 'Trang chủ' , 'tie' ) . '</a></span> ' . $delimiter . ' ';
    ...
    if ( is_category() ) {
      ...
      if ( get_post_type() != 'post' ) {
        ...
        echo $cat_code = str_replace ('View all posts in ','', $cat_code );
			}
		}
        echo $before . get_the_title() . $after;
      }
      ...
      echo $before ;
	  printf( __( 'Kết quả tìm kiếm: %s', 'tie' ),  get_search_query() );
	  echo  $after;
      ...
      echo $before ;
	  printf( __( 'Tác giả: %s', 'tie' ),  $userdata->display_name );
	  echo  $after;
    ...
    if ( get_query_var('paged') ) {
      ...
      echo __('trang ' , 'tie') . ' ' . get_query_var('paged');
      if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
    }
